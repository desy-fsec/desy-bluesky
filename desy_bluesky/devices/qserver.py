...
class QserverDevice(Device):
    ...
    def kickoff(self):
        ...
        elif isinstance(plan, BItem) or isinstance(plan, BPlan) or isinstance(plan, dict):
            self._client.item_add(plan, user_group='primary')
        else:
            raise TypeError(f"Found {type(plan)} in addPlan which is not a valid option to represent a plan for now.")
        ...
        if qserver_status['re_state'] == 'running':
            try:
                ...
                self._client.wait_for_idle_or_paused(
                    timeout=60 * 16)  # wait for the mono to potentially move for 15 mins!
                return status
    ...
    def stop(self):
        ...
        self._client.wait_for_idle_or_paused(timeout=60 * 16)  # wait for the mono to potentially move for 15 mins!
        qserver_status = self._client.status()
        if qserver_status['manager_state'] == 'paused':
            self._client.re_stop()  # equivalent to (RE.stop())
            self._client.wait_for_idle(
                timeout=60)  # this one doesn'y need to be as long because we know the RE is already paused.

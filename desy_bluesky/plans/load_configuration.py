...
def load_configuration(
    ...
    """
    Load configuration into devices. Only devices in the devices dictionary
    passed to the plan. Pass the namespace
    containing your devices to allow all devices to be loaded with
    configuration.

    Parameters
    ----------
    config : Dict[str, Any] | str
        Configuration to load into devices. Can be a dictionary or a string
        path to a yaml file.
    devices : Dict[str, Any]
        Dictionary of devices to load configuration into.
    timeout : float, optional
        Timeout for waiting for devices to finish setting, by default
        DEFAULT_TIMEOUT.
    ...
    if isinstance(config, dict):
        ...
        raise ValueError("config must be a dictionary or a string path to a yaml file.")
    ...
    for device_name, device_configuration in configuration.items():
        try:
            ...
            raise ValueError(
                f"Device {device_name} not found in devices dictionary passed to plan."
            )
        ...
        for field_name, field_value in device_configuration.items():
            try:
                ...
                raise ValueError(
                    f"Field {field_name} not found in device {device_name}."
                )
